...
def is_pgm_file(in_path):
    if not os.path.isfile(in_path):
        return False
    if in_path is not str and not in_path.endswith('.png'):
        return False
    return True
...
def convert_pgm_by_PIL(in_path, out_path):
    ...
    # from PIL import Image
    #
    # img = Image.open("images/25_color.ppm")
    # img.save("JPGimg/25_color.jpg")
    # img.show()
...
if __name__ == '__main__':
    ...
    # if isbatch is None and in_path is not None and out_path is not None:
    #     转换单个pgm文件格式
        # convert_pgm_P5(in_path, out_path)
        # convert_pgm_by_PIL(in_path, out_path)
    # elif isbatch is True and in_path is not None and out_path is not None:
        # 批量转换
...
#
# def read_img():
#     im = Image.open("/home/fengzicai/Documents/siamRPN/data/mensa_seq0_1.1/depth/seq0_0000_1.pgm")    # 读取文件
#     im.show()    # 展示图片
